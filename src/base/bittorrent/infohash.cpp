...
// Register the metatype so `TorrentID` can travel through queued signal/slot
// connections and be stored inside `QVariant`.
...
{
    // Returns a string that is suitable for logging purposes: the v1 hash, the v2
    // hash, or both (comma separated) for a hybrid torrent.
    QString ret;
