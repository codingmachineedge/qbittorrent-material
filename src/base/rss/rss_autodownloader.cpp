    ...
    , m_ioThread {new QThread}
{
    ...
    qCInfo(lcRss) << "Initializing RSS AutoDownloader, processingEnabled=" << isProcessingEnabled();
    ...
    {
        qCCritical(lcRss).noquote() << tr("Couldn't save RSS AutoDownloader data in %1. Error: %2")
    ...
    qCInfo(lcRss) << "RSS AutoDownloader initialized with" << m_rules.size() << "rule(s)";
}

AutoDownloader::~AutoDownloader()
{
    qCDebug(lcRss) << "Destroying RSS AutoDownloader";
    store();
}
...
void AutoDownloader::setRule(const AutoDownloadRule &rule)
{
    ...
    {
        // Insert new rule
        qCInfo(lcRss) << "Adding RSS auto-download rule:" << rule.name();
        setRule_impl(rule);
        sortRules();
        m_dirty = true;
        store();
    ...
    else if (ruleByName(rule.name()) != rule)
    {
        ...
        qCInfo(lcRss) << "Updating RSS auto-download rule:" << rule.name();
        setRule_impl(rule);
        sortRules();
        m_dirty = true;
        storeDeferred();
...
bool AutoDownloader::cloneRule(const QString &ruleName, const QString &cloneRuleName)
{
    ...
    qCInfo(lcRss) << "Cloning RSS auto-download rule" << ruleName << "->" << cloneRuleName;
...
bool AutoDownloader::renameRule(const QString &ruleName, const QString &newRuleName)
{
    ...
    qCInfo(lcRss) << "Renaming RSS auto-download rule" << ruleName << "->" << newRuleName;
...
void AutoDownloader::removeRule(const QString &ruleName)
{
    ...
    qCInfo(lcRss) << "Removing RSS auto-download rule:" << ruleName;
...
{
    qCDebug(lcRss) << "Exporting RSS auto-download rules, format="
                   << ((format == RulesFileFormat::Legacy) ? "Legacy" : "JSON");
    switch (format)
    {
...
{
    qCInfo(lcRss) << "Importing RSS auto-download rules, format="
                  << ((format == RulesFileFormat::Legacy) ? "Legacy" : "JSON");
    switch (format)
    {
    case RulesFileFormat::Legacy:
...
{
    qCInfo(lcRss) << "Setting smart episode filters:" << filters;
...
{
    qCInfo(lcRss) << "Download repacks/propers set to" << enabled;
...
void AutoDownloader::handleTorrentAdded(const QString &source)
{
    ...
    qCDebug(lcRss) << "Torrent added for pending RSS job:" << source;
...
void AutoDownloader::handleAddTorrentFailed(const QString &source, const BitTorrent::AddTorrentError &error)
{
    ...
    qCWarning(lcRss) << "Add torrent failed for pending RSS job:" << source;
    ...
    {
        // TODO(engine): Re-schedule job here.
    }
}
...
{
    qCDebug(lcRss) << "Updating auto-download rules for feed URL change:" << oldURL << "->" << feed->url();
...
void AutoDownloader::processJob(const QSharedPointer<ProcessingJob> &job)
{
    for (AutoDownloadRule &rule : m_rules)
    {
        ...
        qCInfo(lcRss).noquote() << tr("RSS article '%1' is accepted by rule '%2'. Trying to add torrent...")
...
void AutoDownloader::load()
{
    ...
    if (!readResult)
    {
        ...
        {
            qCDebug(lcRss) << "No RSS auto-download rules file found, attempting legacy load";
            loadRulesLegacy();
            return;
        }

        qCWarning(lcRss).noquote() << tr("Failed to read RSS AutoDownloader rules. %1").arg(readResult.error().message);
        return;
    }
...
void AutoDownloader::loadRules(const QByteArray &data)
{
    try
    {
        ...
        for (const auto &rule : rules)
            setRule_impl(rule);
        sortRules();
        qCDebug(lcRss) << "Loaded" << rules.size() << "RSS auto-download rule(s)";
    }
    catch (const ParsingError &error)
    {
        qCCritical(lcRss).noquote() << tr("Couldn't load RSS AutoDownloader rules. Reason: %1")
                .arg(error.message());
    }
}
...
void AutoDownloader::store()
{
    ...
    qCDebug(lcRss) << "Storing" << m_rules.size() << "RSS auto-download rule(s)";
...
{
    qCInfo(lcRss) << "Starting RSS auto-download processing";
    resetProcessingQueue();
...
void AutoDownloader::setProcessingEnabled(const bool enabled)
{
    ...
    {
        qCInfo(lcRss) << "RSS auto-download processing state changed to" << enabled;
