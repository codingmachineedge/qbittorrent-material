...
bool AutoDownloadRule::accepts(const QVariantHash &articleData)
{
    ...
    qCDebug(lcRss) << "Rule" << name() << "accepts article"
                   << articleData[Article::KeyTitle].toString();
...
QJsonObject AutoDownloadRule::toJsonObject() const
{
    ...
    return {{S_ENABLED, isEnabled()}
        ...
        // TODO(engine): The following block is deprecated (legacy compatibility). Replace with the
        // single S_TORRENT_PARAMS field after several releases in 4.6.x.
...
AutoDownloadRule AutoDownloadRule::fromJsonObject(const QJsonObject &jsonObj, const QString &name)
{
    ...
    // TODO(engine): The following block is deprecated (legacy compatibility). Replace with the
    // single S_TORRENT_PARAMS parse after several releases in 4.6.x.
    // === BEGIN DEPRECATED CODE === //
    BitTorrent::AddTorrentParams addTorrentParams;
