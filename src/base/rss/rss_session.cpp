    ...
    , m_workingThread(new QThread)
{
    ...
    qCInfo(lcRss) << "Initializing RSS session"
                  << "processingEnabled=" << isProcessingEnabled()
                  << "refreshInterval(min)=" << refreshInterval()
    ...
    {
        qCWarning(lcRss).noquote() << tr("Couldn't save RSS session configuration. File: \"%1\". Error: \"%2\"")
                .arg(fileName.toString(), errorString);
    });
    ...
    {
        qCWarning(lcRss).noquote() << tr("Couldn't save RSS session data. File: \"%1\". Error: \"%2\"")
                .arg(fileName.toString(), errorString);
    });
    ...
    qCDebug(lcRss) << "RSS working thread started";
    ...
    {
        qCDebug(lcRss) << "Processing enabled — triggering initial refresh";
        refresh();
    }
    ...
    qCInfo(lcRss) << "RSS session initialized:" << feeds().size() << "feed(s) loaded";
}

Session::~Session()
{
    qCDebug(lcRss) << "Deleting RSS session...";

    delete m_itemsByPath[u""_s]; // deleting root folder

    qCDebug(lcRss) << "RSS session deleted";
}
...
{
    qCDebug(lcRss) << "Adding RSS folder:" << path;

    const nonstd::expected<Folder *, QString> result = prepareItemDest(path);
    if (!result)
    {
        qCWarning(lcRss).noquote() << tr("Failed to add RSS folder item. Reason: \"%1\"").arg(result.error());
    ...
    store();
    qCInfo(lcRss) << "RSS folder added:" << path;
    return folder;
}
...
{
    qCDebug(lcRss) << "Adding RSS feed url=" << url << "path=" << path;

    if (m_feedsByURL.contains(url))
    {
        const QString err = tr("RSS feed with given URL already exists: %1.").arg(url);
        qCWarning(lcRss).noquote() << err;
    ...
    if (!result)
    {
        qCWarning(lcRss).noquote() << tr("Failed to add RSS feed item. Reason: \"%1\"").arg(result.error());
        return nonstd::make_unexpected(result.error());
    }
    ...
    qCInfo(lcRss) << "RSS feed added:" << url << "at" << path;
    return feed;
}
...
nonstd::expected<void, QString> Session::setFeedURL(const QString &path, const QString &url)
{
    ...
    if (!feed)
    {
        const QString err = tr("Feed doesn't exist: %1.").arg(path);
        qCWarning(lcRss).noquote() << err;
        return nonstd::make_unexpected(err);
    }
...
nonstd::expected<void, QString> Session::setFeedURL(Feed *feed, const QString &url)
{
    ...
    qCDebug(lcRss) << "Changing feed URL from" << feed->url() << "to" << url;
...
nonstd::expected<void, QString> Session::moveItem(const QString &itemPath, const QString &destPath)
{
    ...
    if (!item)
    {
        const QString err = tr("Item doesn't exist: %1.").arg(itemPath);
        qCWarning(lcRss).noquote() << err;
        return nonstd::make_unexpected(err);
    }
...
nonstd::expected<void, QString> Session::moveItem(Item *item, const QString &destPath)
{
    ...
    qCDebug(lcRss) << "Moving RSS item from" << item->path() << "to" << destPath;

    if (auto *folder = static_cast<Folder *>(item)) // if `item` is a `Folder`
    {
        if (destPath.startsWith(folder->path() + Item::PathSeparator))
        {
            const QString err = tr("Can't move a folder into itself or its subfolders.");
            qCWarning(lcRss).noquote() << err;
            return nonstd::make_unexpected(err);
        }
    }
    ...
    if (!result)
    {
        qCWarning(lcRss).noquote() << result.error();
        return nonstd::make_unexpected(result.error());
    }
    ...
    store();
    qCInfo(lcRss) << "RSS item moved to" << destPath;
    return {};
}
...
nonstd::expected<void, QString> Session::removeItem(const QString &itemPath)
{
    ...
    qCInfo(lcRss) << "Removing RSS item:" << itemPath;
...
void Session::load()
{
    ...
    qCDebug(lcRss) << "Loading RSS session data from" << path.toString();
    ...
    if (!readResult)
    {
        ...
        {
            qCDebug(lcRss) << "No RSS feeds file found, attempting to load legacy configuration";
            loadLegacy();
            store(); // convert to new format
            return;
        }

        qCWarning(lcRss).noquote() << tr("Failed to read RSS session data. %1").arg(readResult.error().message);
        return;
    }
    ...
    {
        qCWarning(lcRss).noquote() << tr("Failed to parse RSS session data. File: \"%1\". Error: \"%2\"")
    ...
    if (!jsonDoc.isObject())
    {
        qCWarning(lcRss).noquote() << tr("Failed to load RSS session data. File: \"%1\". Error: \"Invalid data format.\"")
                .arg(path.toString());
        return;
    }
    ...
    for (const QString &key : asConst(jsonObj.keys()))
    {
        ...
        else if (val.isObject())
        {
            ...
            if (valObj.contains(u"url"))
            {
                ...
                {
                    qCWarning(lcRss).noquote() << tr("Couldn't load RSS feed. Feed: \"%1\". Reason: URL is required.")
                            .arg(u"%1\\%2"_s.arg(folder->path(), key));
                    continue;
                }
                ...
                if (valObj.contains(u"uid"))
                {
                    ...
                    if (uid.isNull())
                    {
                        qCWarning(lcRss).noquote() << tr("Couldn't load RSS feed. Feed: \"%1\". Reason: UID is invalid.")
                                .arg(u"%1\\%2"_s.arg(folder->path(), key));
                        continue;
                    }
                    ...
                    {
                        qCWarning(lcRss).noquote() << tr("Duplicate RSS feed found. UID: \"%1\". Error: Configuration seems to be corrupted.")
                                .arg(uid.toString());
                        continue;
                    }
                }
        ...
        {
            qCWarning(lcRss).noquote() << tr("Couldn't load RSS item. Item: \"%1\". Invalid data format.")
                    .arg(u"%1\\%2"_s.arg(folder->path(), key));
        }
    }
...
void Session::loadLegacy()
{
    ...
    {
        qCWarning(lcRss).noquote() << tr("Corrupted RSS list, not loading it.");
        return;
    }

    qCDebug(lcRss) << "Loading" << legacyFeedPaths.size() << "legacy RSS feed(s)";
...
void Session::store()
{
    qCDebug(lcRss) << "Storing RSS session configuration";
...
void Session::addItem(Item *item, Folder *destFolder)
{
    ...
    destFolder->addItem(item);
    qCDebug(lcRss) << "RSS item registered:" << item->path();
    emit itemAdded(item);
}
...
void Session::setProcessingEnabled(const bool enabled)
{
    ...
    {
        qCInfo(lcRss) << "RSS processing state changed to" << enabled;
...
void Session::setRefreshInterval(const int refreshInterval)
{
    ...
    {
        qCInfo(lcRss) << "RSS refresh interval changed to" << refreshInterval << "minutes";
...
void Session::setFetchDelay(const std::chrono::seconds delay)
{
    ...
    qCInfo(lcRss) << "RSS fetch delay changed to" << static_cast<qint64>(delay.count()) << "seconds";
...
{
    qCDebug(lcRss) << "RSS item about to be destroyed:" << item->path();
...
void Session::handleFeedTitleChanged(Feed *feed)
{
    if (feed->name() == feed->url())
    {
        ...
        qCDebug(lcRss) << "Renaming feed from URL to title:" << feed->title();
...
void Session::setMaxArticlesPerFeed(const int n)
{
    ...
    {
        qCInfo(lcRss) << "RSS max articles per feed changed to" << n;
...
void Session::refresh()
{
    ...
    qCDebug(lcRss) << "Refreshing" << m_refreshTimepoints.size() << "RSS feed(s)";
    ...
    qCDebug(lcRss) << "Next RSS refresh scheduled in" << nextRefreshInterval.count() << "seconds";
}
