...
namespace RSS
{
    ...
    /// Thrown when a rules file cannot be parsed.
    ...
    /// Singleton engine that matches newly fetched `Article`s against the configured
    /// `AutoDownloadRule`s and enqueues matching torrents for adding. Owns the rule
    /// store (CRUD + import/export in JSON or legacy format), smart-episode filters,
    /// and a background IO thread. Bridged to QML via `AutoDownloadRulesModel` /
    /// `RuleEditorController`; subscribe to the signals below, never poll.
